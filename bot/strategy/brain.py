...
BERSERKER_CONFIG = {
    # HEALING
    "HEAL_CRITICAL": 25,        # HP < 25 → HEAL DARURAT! (jangan attack!)
    "HEAL_URGENT": 35,          # HP < 35 → HEAL jika ada waktu
    "HEAL_MODERATE": 50,        # HP < 50 → HEAL jika aman
    
    # FLEE (kabur)
    "FLEE_HP": 20,              # HP < 20 → WAJIB KABUR!
    "FLEE_OUTNUMBERED": 4,      # Musuh > 4 → KABUR!
    "FLEE_STRONG_ENEMY": 30,    # Damage musuh > 30 → KABUR!
    
    # ATTACK (serang)
    "MIN_HP_TO_ATTACK": 35,     # MINIMAL HP 35 untuk attack!
...
def reset_game_state():
    ...
    log.info("=" * 60)
    log.info("⚔️ BERSERKER MODE v2.1 (DENGAN SAFETY CHECK) ⚔️")
...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    ...
    PRIORITY CHAIN:
    1. DEATHZONE ESCAPE
    2. HEALING DARURAT (HP < 25) - SEBELUM ATTACK!
    3. FLEE (jika HP < 20 atau musuh terlalu kuat)
    4. COUNTER ATTACK (hanya jika HP > 35 dan EP cukup)
    5. GUARDIAN FARMING
    6. PICKUP & EQUIP
    7. MODERATE HEALING
    8. EP RECOVERY
    ...
    # Hitung estimasi damage musuh terkuat
    ...
    if region_id in danger_ids:
        safe = _find_safe_region(connections, danger_ids, view)
        if safe and ep >= move_ep_cost:
            log.warning("⚠️ Pending DZ! Escaping")
            return {"action": "move", "data": {"regionId": safe},
                    "reason": "PRE-ESCAPE"}

    # ═══════════════════════════════════════════════════════════════════
    # PRIORITY 2: HEALING DARURAT (SEBELUM ATTACK! INI KUNCI!)
    # ═══════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════
    # PRIORITY 3: FLEE (jika terlalu berbahaya)
    # ═══════════════════════════════════════════════════════════════════
    should_flee = False
    flee_reason = ""
    ...
    # ═══════════════════════════════════════════════════════════════════
    # PRIORITY 4: COUNTER ATTACK (HANYA JIKA HP CUKUP!)
    # ═══════════════════════════════════════════════════════════════════
    ...
    if enemies_here and can_attack:
        ...
        log.warning("⚠️ HP=%d too low to attack! Healing/fleeing instead!", hp)
...
PERUBAHAN PALING PENTING DARI SEBELUMNYA:
1. ✅ HEALING DARURAT DI PRIORITAS #2 (SEBELUM ATTACK!)
2. ✅ MINIMAL HP UNTUK ATTACK = 35 (tidak akan attack jika HP < 35)
3. ✅ CEK DAMAGE MUSUH: kabur jika musuh damage > 30
4. ✅ CEK HP SEBELUM ATTACK: log warning jika HP terlalu rendah

THRESHOLDS AMAN:
- HEAL_CRITICAL = 25 (harus healing!)
- MIN_HP_TO_ATTACK = 35 (tidak attack jika HP < 35)
- FLEE_HP = 20 (kabur jika HP < 20)
- FLEE_STRONG_ENEMY = 30 (kabur jika damage musuh > 30)

Bot sekarang TIDAK akan:
- ❌ Attack jika HP < 35
- ❌ Melawan musuh dengan damage > 30 saat HP < 50
- ❌ Bertahan di area dengan >4 musuh saat HP < 60
"""}
